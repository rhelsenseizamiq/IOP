...
import {
  Row, Col, Card, Statistic, Table, Typography, Spin, message,
  Alert, Timeline, Tag, Tooltip, Progress,
} from 'antd';
import {
  GlobalOutlined, ApartmentOutlined, ClusterOutlined,
...
const CHART    = [ACCENT, 'rgba(99,226,183,0.55)', 'rgba(99,226,183,0.3)', 'rgba(255,255,255,0.15)'];
    ...
    render: (v: number, row: SubnetCritical) => {
      ...
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <Progress percent={v} size="small" strokeColor={utilColor(v)}
            format={(p) => `${p}%`} style={{ flex: 1, margin: 0 }} />
        </div>
      );
    },
  },
...
const DashboardPage: React.FC = () => {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      message.error(e.response?.data?.detail ?? e.message ?? 'Failed to load dashboard');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const alerting   = stats.critical_subnets.filter(s => s.alert_threshold !== null && s.utilization_pct >= s.alert_threshold);
  const piData     = Object.entries(stats.status_breakdown).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value }));
  const inUse      = stats.status_breakdown['In Use'] ?? 0;
  const utilPct    = stats.total_ips > 0 ? Math.round((inUse / stats.total_ips) * 100) : 0;
  const envData    = Object.entries(stats.environment_breakdown).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value }));
  const osData     = Object.entries(stats.os_breakdown).filter(([, v]) => v > 0).map(([name, value]) => ({ name, value }));
  ...
  const STAT_ROWS = [
    [
      ...
      { title: 'Reserved',    value: stats.status_breakdown['Reserved'] ?? 0,  icon: null },
      { title: 'In Use',      value: inUse,                                    icon: null },
    ],
    [
      { title: 'IPv4 Subnets',  value: stats.subnet_v4_count,   icon: <ApartmentOutlined /> },
      { title: 'IPv6 Subnets',  value: stats.subnet_v6_count,   icon: <ApartmentOutlined /> },
      { title: 'VRFs',          value: stats.total_vrfs,         icon: <ClusterOutlined /> },
      { title: 'Aggregates',    value: stats.total_aggregates,   icon: <DatabaseOutlined /> },
    ],
  ];
    ...
    <div>
      ...
      {alerting.length > 0 && (
        <Alert type="warning" showIcon style={{ marginBottom: 16 }}
          ...
          description={alerting.map(s => `${s.cidr} (${s.utilization_pct}% ≥ ${s.alert_threshold ?? 0}%)`).join(' · ')}
        />
      )}
          ...
          {row.map(s => (
            <Col key={s.title} xs={12} sm={6}>
              <Card size="small" styles={{ body: { padding: '12px 16px' } }}>
                <Statistic title={s.title} value={s.value} prefix={s.icon} valueStyle={{ fontSize: 22 }} />
              </Card>
            </Col>
          ))}
        </Row>
        ...
        <Col xs={24} lg={8}>
          <Card title="IP Status" style={{ height: 280 }}>
            <ResponsiveContainer width="100%" height={210}>
              <PieChart>
                <Pie data={piData} cx="50%" cy="45%" innerRadius={55} outerRadius={82} paddingAngle={3} dataKey="value">
                  {piData.map(e => <Cell key={e.name} fill={STATUS_PIE[e.name] ?? DIM} />)}
                </Pie>
                <ReTooltip formatter={(v, n) => [`${v} IPs`, n]} />
                <Legend iconSize={10} formatter={(value, entry) => {
            ...
            <Row gutter={[8, 8]}>
              {[
                ...
                <Col key={item.label} span={12}>
                  <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '12px 8px', textAlign: 'center' }}>
            ...
            {v4total > 0 && (
              <div style={{ marginTop: 12 }}>
                <div style={{ display: 'flex', borderRadius: 4, overflow: 'hidden', height: 6 }}>
                  <div style={{ width: `${(stats.subnet_v4_count / v4total) * 100}%`, background: ACCENT }} />
                  <div style={{ flex: 1, background: 'rgba(255,255,255,0.12)' }} />
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                  <Typography.Text style={{ fontSize: 11, color: DIM }}>IPv4</Typography.Text>
                  <Typography.Text style={{ fontSize: 11, color: DIM }}>IPv6</Typography.Text>
                </div>
              </div>
            )}
          </Card>
        ...
        <Col xs={24} lg={8}>
          <Card title="Overall IP Utilization">
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 8 }}>
              <Progress type="dashboard" percent={utilPct} size={160} strokeColor={utilColor(utilPct)} strokeWidth={10}
                format={p => (
                  <span>
                    <div style={{ fontSize: 26, fontWeight: 700, color: utilColor(utilPct), lineHeight: 1.1 }}>{p}%</div>
                    <div style={{ fontSize: 11, color: DIM, marginTop: 2 }}>In Use</div>
                  </span>
                )}
              />
              <Row gutter={8} style={{ width: '100%', marginTop: 12 }}>
                {[
                  ...
                  { label: 'Reserved', value: stats.status_breakdown['Reserved'] ?? 0 },
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
        <Col xs={24} lg={14}>
          <Card title="IPs by Environment">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={envData} layout="vertical" margin={{ left: 8, right: 40 }}>
                ...
                <Bar dataKey="value" radius={[0, 4, 4, 0]} label={{ position: 'right', fontSize: 11 }}>
                  {envData.map((_e, i) => <Cell key={i} fill={CHART[i % CHART.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </Card>
        ...
        <Col xs={24} lg={10}>
          <Card title="IPs by OS Type">
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={osData} layout="vertical" margin={{ left: 8, right: 40 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="rgba(255,255,255,0.06)" />
                <XAxis type="number" tick={{ fontSize: 11 }} />
                <YAxis type="category" dataKey="name" width={72} tick={{ fontSize: 11 }} />
                <ReTooltip formatter={v => [`${v} IPs`]} />
                <Bar dataKey="value" radius={[0, 4, 4, 0]} label={{ position: 'right', fontSize: 11 }}>
                  {osData.map((_e, i) => <Cell key={i} fill={CHART[i % CHART.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </Card>
        ...
        <Col xs={24} lg={14}>
          <Card title="Subnet Utilization">
            <Table<SubnetCritical>
              dataSource={stats.critical_subnets} columns={criticalColumns}
        ...
        <Col xs={24} lg={10}>
          <Card title="Recent Activity">
            {stats.recent_activity.length === 0
              ...
              : (
                <Timeline items={stats.recent_activity.map((item: ActivityItem) => ({
                  color: (item.action === 'DELETE' || item.action === 'LOGIN_FAILED') ? ERR : ACCENT,
                  children: (
                    <div>
